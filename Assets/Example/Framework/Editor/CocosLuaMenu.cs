...
[InitializeOnLoad]
public static class CocosLuaMenu
{
    //不需要导出或者无法导出的类型
    ...
    public class BindType
    {
        ...
        public BindType(Type t)
        {
            ...
            type = t;                        
            nameSpace = CocosLuaExport.GetNameSpace(t, out libName);
            name = CocosLuaExport.CombineTypeStr(nameSpace, libName);            
            libName = CocosLuaExport.ConvertToLibSign(libName);
            ...
            else
            {
                ...
                wrapName = CocosLuaExport.ConvertToLibSign(wrapName);
            }
        ...
        public static bool IsObsolete(Type type)
        {
            ...
            for (int j = 0; j < attrs.Length; j++)
            {
                ...
                if (t == typeof(System.ObsoleteAttribute) || t == typeof(NoCocosLuaAttribute) || t.Name == "MonoNotSupportedAttribute" || t.Name == "MonoTODOAttribute")
                {
                    return true;
                }
            }
    ...
    public static void GenerateClassWraps()
    {
        ...
        CocosLuaExport.allTypes.AddRange(baseType);

        for (int i = 0; i < list.Length; i++)
        {            
            CocosLuaExport.allTypes.Add(list[i].type);
        }

        for (int i = 0; i < list.Length; i++)
        {
            CocosLuaExport.Clear();
            CocosLuaExport.className = list[i].name;
            CocosLuaExport.type = list[i].type;
            CocosLuaExport.isStaticClass = list[i].IsStatic;            
            CocosLuaExport.baseType = list[i].baseType;
            CocosLuaExport.wrapClassName = list[i].wrapName;
            CocosLuaExport.libClassName = list[i].libName;
            CocosLuaExport.extendList = list[i].extendList;
            CocosLuaExport.Generate(CustomSettings.saveDir);
        }

        Debug.Log("Generate lua binding files over");
        CocosLuaExport.allTypes.Clear();
        allTypes.Clear();        
        AssetDatabase.Refresh();
    }
    ...
    static HashSet<Type> GetCustomTypeDelegates()
    {
        ...
        for (int i = 0; i < list.Length; i++)
        {
            ...
            for (int j = 0; j < fields.Length; j++)
            {
                ...
                if (CocosLuaExport.IsDelegateType(t))
                {
                    set.Add(t);
                }
            }
    ...
    static void GenLuaDelegates()
    {
        ...
        CocosLuaExport.GenDelegates(list.ToArray());
        set.Clear();
        CocosLuaExport.Clear();
        AssetDatabase.Refresh();
    ...
    static CocosLuaTree<string> InitTree()
    {                        
        CocosLuaTree<string> tree = new CocosLuaTree<string>();
        CocosLuaNode<string> root = tree.GetRoot();        
        ...
        {            
            string space = CocosLuaExport.GetNameSpace(dts[i].type, out str);
            AddSpaceNameToTree(tree, root, space);            
        }
    ...
    static void AddSpaceNameToTree(CocosLuaTree<string> tree, CocosLuaNode<string> parent, string space)
    {
        ...
        {
            List<CocosLuaNode<string>> nodes = tree.Find((_t) => { return _t == ns[j]; }, j);

            if (nodes.Count == 0)
            {
                CocosLuaNode<string> node = new CocosLuaNode<string>();
                node.value = ns[j];
            ...
            else
            {
                ...
                for (int i = 0; i < nodes.Count; i++)
                {
                    int count = j;
                    int size = j;
                    CocosLuaNode<string> nodecopy = nodes[i];
                ...
                if (!flag)
                {
                    CocosLuaNode<string> nnode = new CocosLuaNode<string>();
                    nnode.value = ns[j];
                    nnode.layer = j;
                    nnode.parent = parent;
    ...
    static string GetSpaceNameFromTree(CocosLuaNode<string> node)
    {
    ...
    static void GenLuaBinder()
    {
        ...
        allTypes.Clear();
        CocosLuaTree<string> tree = InitTree();        
        ...
        foreach (Type t in set)
        {
            if (null == list.Find((p) => { return p.type == t; }))
            {
                ...
                AddSpaceNameToTree(tree, root, CocosLuaExport.GetNameSpace(t, out libname));
                list.Add(dt);
            }
        }
        ...
        Action<CocosLuaNode<string>> begin = (node) =>
        {
            if (node.value == null)
            {
                return;
        ...
        Action<CocosLuaNode<string>> end = (node) =>
        {
            if (node.value != null)
            {
        ...
        {
            CocosLuaExport.GenEventFunction(dtList[i].type, sb);
        }
    ...
    static void GenRegisterInfo(string nameSpace, StringBuilder sb, List<DelegateType> delegateList, List<DelegateType> wrappedDelegatesCache)
    {
        ...
        for (int i = 0; i < delegateList.Count; i++)
        {
            ...
            Type type = dt.type;
            string typeSpace = CocosLuaExport.GetNameSpace(type, out funcName);

            if (typeSpace == nameSpace)
            {
                funcName = CocosLuaExport.ConvertToLibSign(funcName);
                string abr = dt.abr;
